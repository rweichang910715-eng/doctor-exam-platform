...
function BookExplanation({ q }) {
  ...
  // Use explanation and page list directly from the question database if available
  const hasTextExplanation = !!q.explanation
  const explanationPages = q.explanation_pages || []
  
  // Fallback to explanationsMap if pages are not populated in q yet
  ...
  if (!hasTextExplanation && pagesToRender.length === 0 && !hasCroppedImage) return null
  ...
  return (
    <div className="book-explanation-container" style={{ marginTop: '1rem', borderTop: '1px dashed var(--border-color)', paddingTop: '1rem' }}>
      ...
      {hasTextExplanation && (
        <div className="text-explanation-box" style={{ marginBottom: '1rem' }}>
      ...
      {/* 2. Original Book Scans (except tables etc, can be pasted directly) */}
      {(pagesToRender.length > 0 || hasCroppedImage) && (
        ...
        >
          ...
          >
            ...
            <img 
              ...
              onClick={() => setIsFullScreen(false)}
            />
            <p style={{ color: 'white', marginTop: '1rem', fontSize: '0.9rem', textAlign: 'center' }}>原書第 {activePageNum} 頁 - 2024年醫師國考試題詳解 臨床醫學</p>
          </div>
        </div>
      )}
    </div>
